...
2. **A/V is frame-locked — never let audio and video drift.** `render.py` enforces this; do not reinvent it with a naive `-c copy` concat. The required recipe (proven; deviation reintroduces "the mouth moves at a different rate"): (a) frame-align every cut to the source fps grid; (b) extract VIDEO and AUDIO as *separate* files per segment — video `-frames:v N` (exactly `N=round(dur*fps)` frames), audio PCM `atrim=end_sample=round(N*SR/fps)` (no AAC encoder priming); (c) concat each stream losslessly; (d) ONE final encode that renumbers frames onto the exact grid (`setpts=N/(fps*TB)`, `-fps_mode cfr`) — collapsing the per-boundary timestamp gaps — then composites overlays + subtitles and encodes audio once. Failure modes this defeats: `-ss`+`-r` first-frame offset, per-segment AAC priming, `-frames:v`+`-t` off-by-one, and concat boundary gaps that silently make the video ~29.9fps over a longer span than the audio. After rendering, confirm `avg_frame_rate` is the clean source fps and video-duration ≈ audio-duration (within ~AAC priming, ~20ms).
3. **Fade only where a fade is needed.** A cut that lands in silence gets NO fade (a few-ms declick at most) — fading silence is pointless, and fading a clipped word fragment makes a "weird" swallowed sound. Apply a real fade/crossfade (~30–60ms) ONLY at a hard splice (continuous-speech rejoin). `render.py` reads per-range `fade_in`/`fade_out` (seconds), default a 6ms declick; `place_edges.py` sets these automatically (declick at clean cuts, hard-fade at marked splices). Do NOT blanket-apply 30ms fades to every boundary.
4. **Overlays use `setpts=PTS-STARTPTS+T/TB`** to shift the overlay's frame 0 to its window start. Otherwise you see the middle of the animation during the overlay window.
5. **Master SRT uses output-timeline offsets**: `output_time = word.start - segment_start + segment_offset`. Otherwise captions misalign after segment concat.
6. **Never clip a word — and never trust ASR word times alone to prevent it.** ASR (AssemblyAI especially) inflates word durations, reports onsets early / offsets late, and sometimes collapses several words to one timestamp. So placing an edge at the *reported* word boundary still clips the audible word (the "s" off "tasks", "deadline" cut short) or drops a near-silent word (a final "it."). The deterministic guard, implemented in `place_edges.py`: place clean edges from the AUDIO ENERGY (real onset/offset) **and** clamp to the transcript word extent as a floor/ceiling so a soft word is never dropped — then pad into silence. Run `check_cuts.py`: a CLEAN cut flagged as landing in speech is a word-clip to re-place.
7. **Pad clean edges into silence (~0.15s); tighten long pauses to ~0.3s.** A clean cut should sit in real silence with ~0.15s of room tone kept on each side (so two adjacent cuts leave ~0.3s). `place_edges.py` does this from energy, splitting any pause longer than ~0.4s and dropping runs that are pure silence (ASR inflates the last word into silence, which would otherwise leave a spurious silent segment padding the gap).
...
└── edit/
    ...
    ├── transcripts/<name>.json  ← cached raw AssemblyAI JSON (normalized)
...
- `ASSEMBLYAI_API_KEY` resolves — either in the environment or in `.env` at the video-use repo root. If missing, ask the user to paste one and write it to `.env` (never to the user's `<videos_dir>`).
...
- **`transcribe.py <video>`** — single-file AssemblyAI call. `--num-speakers N` optional. Cached.
- **`transcribe_batch.py <videos_dir>`** — 4-worker parallel transcription. Use for multi-take.
- **`pack_transcripts.py --edit-dir <dir>`** — `transcripts/*.json` → `takes_packed.md` (phrase-level, break on silence ≥ 0.5s).
- **`timeline_view.py <video> <start> <end>`** — filmstrip + waveform PNG. On-demand visual drill-down. **Not a scan tool** — use it at decision points, not constantly.
- **`place_edges.py <coarse_edl.json> -o <edl.json>`** — turns a COARSE edl (the blocks you decided to keep, each `{source,start,end,hard_start?,hard_end?}`) into render-ready ranges with edges placed deterministically from audio energy + transcript word extent: never clips a word, never drops a soft word, lands clean cuts in silence, tightens pauses to ~0.3s, frame-aligns, and sets per-edge fades (declick vs hard-splice). Run this between deciding the cut and rendering — it removes the per-edit guesswork that causes clipped/soft-word/silence bugs. (`--fps`, `--target-sil`, `--split-sil` tunable.)
- **`render.py <edl.json> -o <out>`** — frame-locked render (Hard Rule 2): per-segment dual-stream extract (exact frames + exact audio samples) → lossless concat per stream → ONE final encode that renumbers to true CFR, composites overlays (PTS-shifted) + subtitles LAST, fades only where the EDL marks them. Auto-detects source fps. `--preview` faster, `--draft` 720p, `--build-subtitles` for master.srt. Prints an A/V-lock check at the end — read it.
- **`grade.py <in> -o <out>`** — ffmpeg filter chain grade. Presets + `--filter '<raw>'` for custom.
- **`check_cuts.py <edl.json>`** — the verification "ear": measures audio energy at every cut edge. Classifies flags using the EDL fade — a CLEAN cut (small fade) in speech = a word-clip to re-place (fails the check); a hard-splice cut (larger fade) in speech = intended, just confirm it sounds clean. Catches what a waveform PNG cannot. (Does NOT trust ASR word times — those are unreliable.)
...
5. **Execute.** Decide the COARSE cut: the blocks to keep, in output order, each `{source,start,end,hard_start?,hard_end?}` — mark `hard_start`/`hard_end` True wherever a removed word sits immediately adjacent (a mid-speech stutter/filler/repeat removal). Then run **`place_edges.py`** to turn that into the render-ready `edl.json` (deterministic, word-safe, silence-tightened edges). Drill into `timeline_view` only at genuinely ambiguous decisions. Build animations in parallel sub-agents. Compose via `render.py`. Doing edge placement by hand instead of `place_edges.py` is how clipped/soft-word/silence bugs creep back in.
6. **Preview.** `render.py --preview`.
7. **Self-eval (before showing the user).** First run `check_cuts.py <edl.json>`. **Any WORD-CLIP (a clean cut that landed in speech) must be fixed** — re-run `place_edges.py` (it anchors to the word's audio extent) and re-check; do not ship a clip. HARD-SPLICE flags are intended rejoins — confirm they sound clean. (The agent cannot literally hear audio; this is the objective proxy. The final subjective listen is the human's.) Then run `timeline_view` on the **rendered output** (not the sources) at every cut boundary (±1.5s window). Check each image for:
   - Visual discontinuity / flash / jump at the cut
   - Waveform spike at the boundary (audio pop)
   - Subtitle hidden behind an overlay (Rule 1 violation)
   - Overlay misaligned or showing wrong frames (Rule 4 violation)

   Also sample: first 2s, last 2s, and 2–3 mid-points — check grade consistency, subtitle readability, overall coherence. **Verify A/V lock** from `render.py`'s end-of-run report: `avg_frame_rate` must equal the source fps and video-duration ≈ audio-duration; if not, the sync is broken — do not ship. Run `ffprobe` to confirm duration matches the EDL.
...
- **Intra-phrase edits (stutters, fillers, doubled words, mid-sentence corrections, tics, bleeps).** These all share one shape: the unit is smaller than a phrase and usually sits in zero-gap speech. Mark the removal's edges `hard_start`/`hard_end` in the coarse EDL and let `place_edges.py` handle the boundary; verify with `check_cuts.py`. (Pad toward swallowing the removed word, never toward kept content.)
- **Hunt repeats and restarts deliberately — they are the easiest thing to leave in by accident.** Read the FULL verbatim transcript (`disfluencies` are kept as words) and cut: doubled lead-ins ("so… so", "and… and"), restarted clauses where the speaker says nearly the same words twice ("here are the exact… here are the exact steps", "you may have heard— you may have heard"), abandoned false starts (often end on `—`), and self-corrections ("that's not 7:30… so 7:30"). When a phrase contains a `—` or repeats words, it almost always needs a cut. Keep the *cleaner/more complete* instance, drop the other. Also decide on conversational fillers the user dislikes ("I guess", "like", "you know", "uh") — ask their tolerance in the strategy step; in "clean" mode cut the clear ones, not every instance.
- **Hard-splice residue.** Removing a word from zero-gap speech can leave a half-syllable on the kept side (an "extra sound" after the rejoin). Mark it hard, let `place_edges.py` swallow toward the removed word, and listen-check via `check_cuts.py`; if a HARD-SPLICE flag has high energy, nudge the edge a few ms further into the removed word.
...
- **Running Whisper locally on CPU.** Slow and it normalizes fillers. Use hosted AssemblyAI.
